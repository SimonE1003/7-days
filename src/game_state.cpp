...
void display_status(GameState gs)
{
    ...
    mvprintw(9, center, "Water: %d\n", gs.water);    mvprintw(10, center, "Health: %d\n", gs.health);
    ...
    {
	attron(COLOR_PAIR(1));
        mvprintw(14, center, "You are ill!!!");
	attroff(COLOR_PAIR(1));

    }
    int prtcnt = 0;
    mvprintw(15, center+prtcnt, "Items: ");
    prtcnt += 7;
    for (string item : gs.items)
    {
        
        mvprintw(15, center+prtcnt, "%s", item.c_str());
        prtcnt += item.size() + 2;

    }
    mvprintw(17, center, "Press 'q' to quit\n");
    refresh();
